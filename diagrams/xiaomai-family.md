# 小麦一家是怎么运转的

> 一张图看懂这个服务器上的 AI 家庭:谁是谁、各自想干什么、什么时候才动用 AI 模型。
> 数据是 **2026-09-18 从线上实际运行的配置里读出来的**,不是凭文档画的(见文末「这张图的依据」)。

## 全家总览

```mermaid
mindmap
  root((小麦一家))
    家里有谁
      👴 老麦 · 爷爷
        AI 村民,住在出生点
        是游戏插件,不是玩家
      👨 小麦 · 丈夫 ✅ 在线
        出门干活
        会 19 个动作
      👩 麦娘 · 妻子 ✅ 在线
        守家、种地、管粮食
        会 15 个动作
      👦 麦豆 · 儿子 ✅ 在线
        跟着大人学,在家附近玩
        会 6 个动作
    脑子分四层
      ① 反射 · 每秒一次
        掉血、落水、岩浆、窒息
        当场处理,不问模型
      ② 紧急规则
        快饿死 → 先吃
        手里没武器 → 做把剑
        天黑 → 回家睡
        离家太远 → 传送回家
      ③ 脚本 · 主力
        大目标 → 小目标 → 一步一步做
      ④ 兜底
        一件活都开不了工时
        才让模型挑一个动作
    各自的大目标
      小麦
        活着
        家和家人是安全的
        房子像个家
        手上有家伙
        家里不缺木料
      麦娘
        活着
        全家有饭吃
        有一块自己的地
        家当都收进箱子
      麦豆
        别死,别走丢
        学会爸爸会的事
    什么时候才问 AI 模型
      能开工的活只有 1 件 → 直接干,不问
      有好几件可选 → 才问模型挑
      做没做完 → 代码说了算
      能不能开工 → 代码说了算
      同一件活失败 3 次 → 记下来,冷却一阵
    老麦怎么说话
      有人找他
        聊天里喊「老麦」
        或者站在他身边 12 格内
        本服的问题 → 标准答案原样回
        其它问题 → 模型想 + 联网查
      网页地图上有人问 → 也会回
      自己闲聊
        每 6 到 12 分钟一句
        是写好的台词,不耗模型
        ✅ 没有真人在线就不说,也不点机器人的名
        ✅ 机器人跟他说话一律不理
    全家共用的
      家里的箱子在哪、里面有什么
      地盘在哪、谁能进
      各自私有:家、房子、经验、做过的事
    你能管的
      后台开关每一个脚本
      调整先做哪个
      ⏸ 暂停中:圈地、授权家人进地盘
      一个开关退回旧版
```

## AI 玩家每一轮是怎么决定干什么的

小麦、麦娘、麦豆三个跑的是**同一份代码**,只是职业不同,所以决策流程一模一样。
三个都在线(2026-09-18 起)。

```mermaid
flowchart TD
    A([新的一轮]) --> B{有危险吗?<br/>掉血 · 落水 · 岩浆 · 窒息}
    B -- 有 --> R[① 反射<br/>当场处理,不问模型]
    B -- 没有 --> C{有紧急的事吗?<br/>快饿死 · 没武器 · 天黑 · 离家太远}
    C -- 有 --> H[② 紧急规则先处理<br/>手上的活不丢,处理完接着干]
    C -- 没有 --> D{手上有活吗?}
    D -- 有 --> E{做完了吗?<br/>由代码判断}
    E -- 没做完 --> F[做下一步]
    F -- 同一步连败 3 次 --> K[放下这件活<br/>记进履历,冷却一阵]
    E -- 做完了 --> G
    D -- 没有 --> G{现在能开工的活有几件?<br/>由代码算出来}
    G -- 0 件 --> Z[④ 兜底<br/>让模型直接挑一个动作]
    G -- 1 件 --> I[直接开工<br/>一次模型都不问]
    G -- 好几件 --> M[问模型挑一件<br/>同时告诉它最近做过什么、栽过什么]
```

**这个设计最重要的一条:凡是代码能确定的事,就不问模型。**
模型只做它擅长的一件事 —— 在几个都能做的选项里挑一个。
它不可能挑出做不到的事,因为做不到的根本不会出现在候选名单里。

## 每个人具体会做哪些事

| 大目标 | 小麦 👨 | 麦娘 👩 | 麦豆 👦 |
|---|---|---|---|
| **活着** | 交给反射和紧急规则 | 交给反射和紧急规则 | 天黑或受伤就回家 |
| **家和家人安全** | 天黑回家睡觉 · 赶走家门口的怪 · ⏸圈地 · ⏸授权家人 | | |
| **房子像个家** | 造工作台和箱子 · 凑盖房材料 · 从箱子取材料 · 把房子盖完 | | |
| **手上有家伙** | 做一把剑 | | |
| **家里不缺木料** | 砍树 · 把木料存进箱子 · 出去找林子 | | |
| **全家有饭吃** | | 收割熟了的庄稼 · 把小麦做成面包 · 粮食存进粮箱 · 做粮箱 | |
| **有块自己的地** | | 做锄头 · 耕地 · 从箱子拿种子 · 割草弄种子 · 播种 | |
| **家当收进箱子** | | 把家当收进箱子 · 原木做成木板 | |
| **学会爸爸会的事** | | | 在家附近玩 · 跟爸爸去看砍树 |

⏸ = 暂停中(Owner 2026-09-18 决定「暂时先别圈」)。全部 28 个脚本,开着 26 个。

---

## 这张图的依据

画之前对照了线上实际运行的东西,而不是只看说明文档 —— 说明文档里已经有两处过时了:

| 项目 | 说明文档(server/11) | 线上实际 |
|---|---|---|
| 小麦会几个动作 | 18 | **19** |
| 麦娘会几个动作 | 9 | **15**(昨晚新学了种地那一套) |
| 麦豆 | 列在表里 | ~~已设计、没在运行~~ → **2026-09-18 已上线**(见下) |

- 家庭关系、人格、动作清单 ← `roles.json`
- 大目标、脚本、开关状态 ← `scripts.json`
- 谁在运行 ← 服务器上实际的进程
- 老麦的规则 ← 老麦插件的配置(触发方式、12 格范围、闲聊间隔)

### 麦豆是怎么上线的(2026-09-18)

照着麦娘的样子加了一套运行配置,**一行代码没改**:

| | 麦娘 | 麦豆 |
|---|---|---|
| 职业 | wife | child |
| 数据目录 | `data/wife` | `data/child` —— 必须隔开,否则一起来就会覆盖小麦的存档 |
| 最短思考间隔 | 20 秒 | **30 秒**(家里最小的,不跟大人抢算力) |
| 进程优先级 | Nice 10 | Nice 15 |

上线时处理了两件事:

- **麦豆不会「安家」**(那个动作不在它的清单里),而代码只在「还没有家」时才去安家 ——
  所以它会永远回不了家。处理:**孩子跟爸妈住**,把小麦那份家(Owner 亲手定的)给了它。
- 它第一次登录出生在世界出生点,离家 146 格,触发「离家太远就传送」,
  但它在服务器那边**没登记过这个家**,传送失败。
  处理:先送它回家,再用 `/sudo MaiDou sethome base` 让它自己在家门口登记 ——
  和小麦、麦娘用的是同一套机制。

上线后确认过:代码里**没有写死任何人名**(家人名单从角色表读,它被自动认成家人);
**公屏发言是代码统一管的**,它人设里写着「话多一点」,但只有被点名才回,上线后公屏说话 0 句;
按 Owner「暂时先别圈地」的决定,它的圈地权限也和小麦、麦娘一样关掉了。

### 关于老麦的闲聊(2026-09-18 查的)

Owner 觉得「没真人的时候老麦还在说话,浪费算力」。查下来:

```
老麦说的 164 句里
├ 写好的闲聊台词(不调用模型)   163 句   其中对着空服务器说的 56 句
└ 模型现场想出来的回答           1 句
```

**闲聊几乎不耗算力** —— 那些是预先写好的句子。但它确实该关:
台词里「点名某个在线玩家」的功能会把机器人也当成玩家,
于是没真人的时候,它一直在对小麦和麦娘说「MaiNiang 你好」「你的房子圈领地了没」。
**已改好(McAI 1.8.0,2026-09-18 上线并带对照验证):没有真人在线就不闲聊,点名只点真人,机器人跟它说话一律不理。** 孩子找它说话、从网页上问,照常回答。谁是机器人,读的是机器人自己的 `roles.json`,全服只有这一份名单。

> 数这个数的时候,第一版把「午夜前就在线的人」漏掉了 ——
> 今天的日志从午夜才开始,那几行「上线」记在昨天的日志里。
> 跨午夜把两天日志接起来,才是上面这个数。
> (同一类错误见 [docs/26](../docs/26-that-was-not-a-control-group.md))

**架构细节**见 [server/11 · AI 玩家的脑子是怎么运转的](../server/11-ai-player-brain.md);
**每一条设计为什么这么定**见 [server/12 · 怎么驱动 AI 玩家](../server/12-driving-ai-players.md)。
