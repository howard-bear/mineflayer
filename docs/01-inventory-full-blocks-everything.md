...
1. **屏蔽逻辑和执行器不能只是"用同一个判据",必须是"同一个函数"。**
   ...
   **第五次**出现在盖房子的站位算法里,而这次是**在部署之前**抓到的:
   `findStandSpot`(选站哪儿)会把**目标格自己**选成站位,
   于是 `placeAt`(放方块)必然回一句"那是我自己站着的地方" —— **那一格永远放不上**。
   两个函数对"身体占了哪些格"各有一套判断,又漂了。

   抓法值得单独说:**把待部署的那版抽出来,喂一个假的 5×5 房子跑探针**,
   不连服务器、不碰生产:

   ```
   16 个探针:8 次选中目标格本身、12 次身体占住还没砌的墙格
   抽出共用判据 bodyOccupies() + 加 forbid 排除未砌格之后:0/16
   ```

   > **"两份代码会漂"这件事可以离线复现。** 构造一个小场景、把两边各跑一遍、
   > 看它们会不会给出不一致的答案 —— 比部署上去等日志快得多,也安全得多。
